...
public class ChargeMoneyServlet extends HttpServlet {
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        double money_to_charge = Double.parseDouble(req.getParameter(Constants.CHARGE_MONEY));
        if(usernameFromParameter != null) {
            status = ContextListener.engine.loadMoneyToCustomerAccount(usernameFromParameter, money_to_charge);
        }
        if (status == 200) {
